...
PLUGIN_EMPTY_RESPONSE: ErrorCode = ErrorCode(BizCode.LocalErr, _("插件返回数据为空"))
WEB_EXEC_ELE_ERROR_FORMAT: ErrorCode = ErrorCode(BizCode.LocalErr, _("插件执行错误: {}"))
...
# ==================== 元素定位相关错误 ====================
WEB_GET_ELE_ERROR_FORMAT: ErrorCode = ErrorCode(BizCode.LocalErr, _("网页元素查找失败 {}"))
...
SLIDER_ELEMENT_NOT_UNIQUE: ErrorCode = ErrorCode(BizCode.LocalErr, _("滑块元素定位不唯一，请检查！"))
TRACK_ELEMENT_NOT_UNIQUE: ErrorCode = ErrorCode(BizCode.LocalErr, _("滑轨元素定位不唯一，请检查！"))

# ==================== 文件上传下载相关错误 ====================
DOWNLOAD_WINDOW_NO_FIND: ErrorCode = ErrorCode(
    BizCode.LocalErr, _("未弹出下载窗口，可以尝试用模拟人工点击或设置浏览器始终弹出下载窗口")
)
DOWNLOAD_TIMEOUT: ErrorCode = ErrorCode(BizCode.LocalErr, _("等待下载完成超时"))
UPLOAD_WINDOW_NO_FIND: ErrorCode = ErrorCode(
    BizCode.LocalErr, _("未弹出上传窗口，可以尝试用模拟人工点击或设置浏览器始终弹出上传窗口")
)
UPLOAD_PATH_INVALID: ErrorCode = ErrorCode(BizCode.LocalErr, _("请提供正确的url链接和文件存储路径"))
FILE_PATH_ERROR_FORMAT: ErrorCode = ErrorCode(BizCode.LocalErr, _("{}表格文件路径错误，仅支持 .xlsx 文件"))

# ==================== 表格筛选相关错误 ====================
FILTER_CONDITION_NOT_SUPPORTED_FORMAT: ErrorCode = ErrorCode(BizCode.LocalErr, _("暂不支持该筛选条件：{}"))
FILTER_CONDITION_INVALID: ErrorCode = ErrorCode(BizCode.LocalErr, _("条件异常，请输入正确的条件！"))
DATA_PROCESS_MISSING_PARAM_FORMAT: ErrorCode = ErrorCode(BizCode.LocalErr, _("第{}列数据处理缺少参数"))
DATA_PROCESS_PARAM_INVALID_FORMAT: ErrorCode = ErrorCode(BizCode.LocalErr, _("参数异常，请输入正确的参数！{}{}"))
