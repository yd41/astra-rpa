...
def test_ocr_vat_invoice_single(mock_post):
    mock_post.return_value = {"payload": {"invoice_no": "12345678"}}
    with patch("astronverse.openapi.ocr._common.utils.generate_src_files", return_value=["/fake/invoice.jpg"]):
        with patch("builtins.open", create=True) as mock_file:
            ...
            result = ocr_vat_invoice(src_file="/fake/invoice.jpg", is_save=False)
    assert len(result) == 1
    mock_post.assert_called_once()


@patch("astronverse.openapi.ocr._common.GatewayClient.post_multipart")
def test_ocr_vat_invoice_raises_on_empty_file(mock_post):
    with patch("astronverse.openapi.ocr._common.utils.generate_src_files", return_value=[]):
        with pytest.raises(Exception):
            ocr_vat_invoice(src_file="", is_save=False)
